...
//document.cookie = 'alert' + "=" + 'no' + "; " + expires;
	...
	jQuery('#subscribe_campaign').click(function(){
...

      {
		...
		var url=jQuery('#server_url').val();

		

	

		 data = data + '&action=my_action&url=' + url;
...

         {
			...
			if(jQuery('#substype').val()==2){
					...
					if(jQuery('#subs').val()==''){
						...
						window.location.replace(jQuery('#subs').val());
...

         {
			...
			window.location.replace(admin_url+"/admin.php?page=masta-lists&action=view_list&filter_list="+list_id);
