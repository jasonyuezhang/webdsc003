...
//echo $newstring;
global $wpdb;
$responder_id  = trim($m_st['camp_id']);
...
$rs_data = $wpdb->get_results( "SELECT * FROM $masta_responder WHERE responder_id = $responder_id");
				
				
			
			$campaing_data = $rs_data[0];
			$report_id = $m_st['report_id'];	
			$subscriber_id = $m_st['subscriber_id'];
			//echo $m_st['subscriber_id'];
			$list_id  = trim($campaing_data->to_list);
			$open_url = home_url().'?openid2='.urlencode(mmasta_encrypt($report_id));
			...
			$contentbody=strtr($campaing_data->resp_mail,$arrr);
		...
		//	echo $plainTextBody;
			...
			$plainTextBody=$plainTextBody."<div style='text-align: center; margin-top: 25px;'><img src='".plugins_url()."/mail-masta/lib/css/images/mailmasta-finalmail.png'></div>";
			}
			...
			if($api_send_type == 1) {
				...
				mail($m_st['test_mail'], $send_subject, $send_message, $headers);
			}
