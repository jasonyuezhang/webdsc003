...
abstract class TimetableTime
{
  ...
  public static function renderHourSelectList( $name, $default = NULL )
  {
    ...
    $increment = get_option( 'timetable_time_increments', 15 );
    ...
    $minutes = TimetableHtml::generateSelectList( $minutes_array, array( 'name' => $name . '_minutes' ), FALSE, $default_min );
    
    if ( $is_24 ) {
      /* 24 hour mode */
      $hours = TimetableHtml::generateSelectList( range( 0, 23 ), array( 'name' => $name . '_hours' ), FALSE, $default_hour );
      $form = "<td>$hours</td><td>$minutes</td>";
    }
    else {
      /* AM/PM mode */
      $hours = TimetableHtml::generateSelectList( range( 1, 12 ), array( 'name' => $name . '_hours' ), FALSE, $default_hour );
      $am_pm = TimetableHtml::generateSelectList( array( 'AM', 'PM' ), array( 'name' => $name . '_ampm' ), FALSE, $default_ampm );
  ...
  /**
   * Converts a time string into TIMETABLE database time format.
   * 
   * @param string $time
   */
