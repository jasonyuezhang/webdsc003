...
class TimetableForm
{
  ...
  function __construct( $form_id )
  {
    ...
    $this->_base_name = strtolower( str_replace( 'Timetable', '', $form_id ) );
    $this->_table_name = $wpdb->prefix . 'timetable2_' . $this->_base_name;
    $this->_class_name = 'Timetable' . ucwords( $this->_base_name );
  }
  ...
  {
    $class = $model . 'Form';
    require_once TIMETABLE_PLUGIN_DIR . '/models/' . $class . '.php';
  ...
  protected static function setFormError( $form_errors = array() )
  {
    if ( ! empty( $form_errors ) ) {
      TimetableHtml::show_wp_message( $form_errors );
    }
  }
