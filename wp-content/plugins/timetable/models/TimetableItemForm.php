<?php
/**
 * Defines the TimetableItemForm class.
 */

class TimetableItemForm extends TimetableForm implements ITimetableForm
{
  /**
  ...
  public function begin( $op, $id )
  {
    ...
    // TODO: Remove this
    TimetableIOS::updateItemJson( $this->_base_name );
  }
    ...
    elseif ( $op == 'delete_items') {
      ...
      foreach ( $_POST as $key => $value ) {
        if ( preg_match( "/^delete_[0-9]+$/", $key ) == 1 ) {
          ...
          $schedule_entries = TimetableSchedule::model()->getByAttributes( array( $col => $value ) );
          if ( $schedule_entries ) {
