...
/**
 * @file
 * Defines the TimetableIOS model class. 
 */
...
class TimetableIOS
{
  ...
  public static function updateScheduleJson()
  {
    ...
    $multiDimArray = TimetableSchedule::getClassesMultiDimArray( NULL, $ios = TRUE );
    ...
    $fp = file_put_contents(TIMETABLE_PLUGIN_DIR . '/ios/schedule.json', $output);
    
    if ( $fp > 0 ) {
      self::updateItemJson( 'class' );
      self::updateItemJson( 'instructor' );
      self::updateItemJson( 'classroom' );
      self::updateTimetableJson();
    }
  }
  ...
  {
    global $wpdb;
    $table_name = $wpdb->prefix . 'timetable2_' . $item;
    $schedule_table = $wpdb->prefix . 'timetable2_schedule';
    $item_id = $item . '_id';
    ...
    $fp = file_put_contents( TIMETABLE_PLUGIN_DIR . "/ios/$item.json", $output );
    
    if ( $fp > 0 )
      self::updateTimetableJson( $item );
  }
  
  /**
   * Generate timetable.json which contains the site's name and a last_updated timestamp.
   */
  public static function updateTimetableJson()
  {
    $settings = array(
    ...
    $fp = file_put_contents( TIMETABLE_PLUGIN_DIR . "/ios/timetable.json", $output );
  }
