 ...
 */
class TimetableSchedule extends TimetableActiveRecord
{
  public $_tableName;
  ...
  private function tableName()
	{
		global $wpdb;
		return $wpdb->prefix . 'timetable2_schedule';
	}

	/**
	 * Returns the week days array arranged as per the options 'timetable_first_day_of_week'
	 * and 'timetable_number_of_days.
	 *
	...
	public static function getWeekDaysArray( $ignore_number_of_days = FALSE, $short_names = FALSE )
	{
	  ...
	  $start_day = get_option( 'timetable_first_day_of_week', 1 );
	...
	{
	  if ( is_array( TimetableSchedule::model()->getCol( 'weekday' ) ) )
	   $db_weekdays = array_unique( TimetableSchedule::model()->getCol( 'weekday' ) );
	  else
	    return;
	...
	{
	  $is_24_mode = ( get_option( 'timetable_24_hour_mode' ) == 'yes' ) ? TRUE : FALSE;
	  date_default_timezone_set( TimetableTime::getDefaultTimezone() );
	  ...
	  if ( $classroom == NULL ) {
	    ...
	    $classroom_id = TimetableClassroom::model()->getByAttribute( 'classroom_name', $classroom )->id;
	  ...
	  $weekdays = array_flip( TimetableSchedule::model()->getWeekDaysArray() );
	  ...
	  foreach ( $hours as $key => $value ) {
	    ...
	    if ( get_option( 'timetable_use_timezones' ) == 'yes' ) {
	 ...
	 * $array[weekday][start_hour][class_id][ (TimetableSchedule) class].
	 *
	 * @parram string $classroom
	 * 	If set, only the entries for the specified classroom will be retrieved.
	 */
	...
	{
	  $default_timezone = TimetableTime::getDefaultTimezone();
	  ...
	  if ( $classroom == NULL )
	    ...
	    $classroom_obj = TimetableClassroom::model()->getByAttribute( 'classroom_name', $classroom );
	...
	{
	  $class = TimetableClass::model()->getById( $this->class_id );
   ...
   * Returns an instructor name based on the instance instructor_id property.
	...
	public function getInstructorName( $strip_slashes = FALSE ) {
	  $instructor = TimetableInstructor::model()->getById( $this->instructor_id );
	...
	public function getClassroomName( $strip_slashes = FALSE ) {
	  $classroom = TimetableClassroom::model()->getById( $this->classroom_id );
   ...
   */
	private static function checkItemCollision( $item_base_name, $item, $start_hour, $end_hour, $weekday )
	{
	  ...
	  $class_name = 'Timetable' . ucwords( $item_base_name );
	  $instance = new $class_name();

	  $item = $instance->getByAttribute( $item_base_name . '_name', $item );
	  $item_entries = TimetableSchedule::model()->getRowsByAttribute( $item_base_name . '_id', $item->id );
	  ...
	  if ( ! empty( $item_entries)) {
	    foreach ( $item_entries as $entry ) {
	      ...
	      /* Collision algorithm */
	      if ( ! isset( $_GET['timetableid'] ) || $entry->id != $_GET['timetableid'] ) {
	...
	/**
	 * Generates a TIMETABLE table cell.
	 *
	 * @param array $classes
	 ...
	 *
	 * @see TimetableSchedule::model()->getClassesMultiDimArray()
	 */
	public static function renderClassTd( $classes, $start_hour, $weekday, $layout = 'vertical', $start_hours_array = NULL )
	{
	  $output = '';
	  $format_hour = TimetableTime::convertTimeToDbFormat( $start_hour );
    ...
    foreach ( $weekdays_flipped as $key => $value ) {
	  ...
	  if ( isset( $classes[$weekday][$format_hour] ) ) {
	    $output .= "<td class='timetable-schedule-cell active $weekday_css col-$col $addon_css'>";
	    $output .= "<div class='timetable-active-class-container'>";
	    ...
	    foreach ( $classes[$weekday][$format_hour] as $class ) {
	      ...
	      $class_description = esc_html( stripslashes( TimetableClass::model()->getById( $class->class_id )->class_description ) );

	      $instructor = esc_html( stripslashes( $class->getInstructorName() ) );
	      $instructor_description = esc_html( stripslashes( TimetableInstructor::model()->getById( $class->instructor_id )->instructor_description ) );
	      ...
	      $details_class = "<a class='timetable-qtip' name='$class_description'>$class_name</a>";
	      $final_details = str_replace( '[class]', $details_class, $default );

	      $details_instructor = "<a class='timetable-qtip' name='$instructor_description'>$instructor</a>";
	      ...
	      $output .= "<div class='timetable-active-div-$i timetable-active-div $odd_even'>";
	      $output .= "<p>$class_name</p>";
	      $output .= "<div class='timetable-class-details'><p>$final_details</p></div>";
	      $output .= '</div>';
	    ...
	    $output .= "<td class='timetable-schedule-cell $weekday_css col-$col $addon_css'></td>";
	  }
