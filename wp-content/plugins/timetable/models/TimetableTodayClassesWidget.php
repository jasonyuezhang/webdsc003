...
/**
 * @file
 * Defines the TIMETABLE Today's Classes widget.
 */
...
class TimetableTodayClassesWidget extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
	 		'timetable_today_classes_widget', // Base ID
			__( "TIMETABLE Today's Classes", 'weekly-class-schedule' ), // Name
	...
	public function widget( $args, $instance ) {
	  ...
	  extract( $args );
		...
		if ( isset( $classroom ) && $classroom != 'all' ) {
		  $classroom = TimetableClassroom::model()->getByAttribute( 'classroom_name', $classroom );
		  $classes = TimetableSchedule::model()->getByAttributes( array( 'weekday' => $today, 'classroom_id' => $classroom->id, 'visibility' => '1'  ), array( 'col' => 'start_hour', 'order' => 'ASC' ) );
		}
		else {
		  $classes = TimetableSchedule::model()->getByAttributes( array( 'weekday' => $today, 'visibility' => '1' ), array( 'col' => 'start_hour', 'order' => 'ASC' ) );
		}
		...
		if ( ! empty( $classes ) ) {
		  ...
		  foreach ( $classes as $class ) {
			...
			$end_hour = $class->getEndHour();
		    echo "<li><ul><li>$class_name</li> <li>$start_hour - $end_hour</li></ul></li>";
		  }
	...
	public function form( $instance ) {
	  ...
	  if ( isset( $instance[ 'title' ] ) ) {
		...
		$classrooms = TimetableClassroom::model()->getAllRecords();
