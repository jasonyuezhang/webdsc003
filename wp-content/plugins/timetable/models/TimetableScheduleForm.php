...
/**
 * @file
 * Defines the TimetableScheduleForm class.
 */

class TimetableScheduleForm extends TimetableForm implements ITimetableForm
{
  /**
  * @see ITimetableForm::begin()
  */
  public function begin( $op, $id )
  {
    ...
    // TODO: Remove this
    TimetableIOS::updateScheduleJson();
  }

  /**
  * @see ITimetableForm::validate()
  */
  public function validate( $op, $id )
  {
    ...
    switch ( $op ) {
      ...
      case 'add_item':
        ...
        $options['class_select'] = TimetableClass::model()->getCol( 'id' );
        $options['instructor_select'] = TimetableInstructor::model()->getCol( 'id' );
        $options['classroom_select'] = TimetableClassroom::model()->getCol( 'id' );
        $options['weekday_select'] = array_flip( TimetableSchedule::model()->generateWeekdays() );
        ...
        $timezone = ( isset( $_POST['timezone'] ) ? $_POST['timezone'] : TimetableTime::getDefaultTimezone() );

        $time_logic_validation = TimetableSchedule::model()->validateTimeLogic( $start_hour, $end_hour, $instructor, $classroom, $weekday, $timezone );
    ...
    if ( ! empty( $form_errors ) ) {
      TimetableForm::setFormError( $form_errors );
      return;
    }
  ...
  /**
  * @see ITimetableForm::process()
  */
  public function process( $op, $id )
  {
    if ( $op != 'delete_items') {
      $class = TimetableClass::model()->getByAttribute( 'id', $_POST['class_select'] );
      $instructor = TimetableInstructor::model()->getByAttribute( 'id', $_POST['instructor_select'] );
      $classroom = TimetableClassroom::model()->getByAttribute( 'id', $_POST['classroom_select'] );
      $weekday = $_POST['weekday_select'];

      if ( isset( $_POST['start_hour_ampm'] ) ) {
        $start_hour = TimetableTime::prepareHour( $_POST['start_hour_hours'], $_POST['start_hour_minutes'], $_POST['start_hour_ampm'] );
      }
      else {
        $start_hour = TimetableTime::prepareHour( $_POST['start_hour_hours'], $_POST['start_hour_minutes'] );
      }

      if ( isset( $_POST['end_hour_ampm'] ) ) {
        $end_hour = TimetableTime::prepareHour( $_POST['end_hour_hours'], $_POST['end_hour_minutes'], $_POST['end_hour_ampm'] );
      }
      else {
        $end_hour = TimetableTime::prepareHour( $_POST['end_hour_hours'], $_POST['end_hour_minutes'] );
      }
      ...
      elseif ( $op == 'save_item' ) {
        $record = TimetableSchedule::model()->getById( $id );
      }
    ...
    elseif ( $op == 'delete_items') {
      ...
      if ( $count > 0 ) {
        ...
        TimetableHtml::show_wp_message( $message, 'updated' );
      }
    }
    TimetableIOS::updateScheduleJson();
  }
