...
class URE_GF_Access {
    ...
    public function __construct(Ure_Lib $lib) {
        ...
        $this->count_forms_query = "SELECT count(0) as count FROM {$this->form_table_name} UNION ALL SELECT count(0) as count FROM {$this->form_table_name} WHERE is_active=1 ";
...
<?php                    
        ...
        if (count($allowed_forms)>0) {
            ...
            if (substr(trim($query), 0, 11)==='SELECT f.id') {                   
                ...
                $query = "SELECT count(0) as count 
                              ...
                              WHERE id in ($allowed_forms_str)
                          UNION ALL 
                            SELECT count(0) as count FROM $this->form_table_name 
                                WHERE is_active=1 and id in ($allowed_forms_str)";
            }
        }                        
    ...
    public function restrict_form_list($query) {
        ...
        if (is_blog_admin()) {  // if not admin dashboard - nothing to change
            ...
            // set filter for dashboard GF widget queries at forms_model.php, v. 1.7.7 get_form_summary(), line # 163
