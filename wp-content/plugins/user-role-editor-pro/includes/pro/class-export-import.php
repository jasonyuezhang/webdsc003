...
?>
                  ...
                  <button id="ure_export_roles" class="ure_toolbar_button" title="Export All Roles to your local disk">Export</button> 
                  <button id="ure_import_roles" class="ure_toolbar_button" title="Import Roles from your local disk">Import</button>
<?php
...
<?php
    ...
    public function add_js() {
        ...
        wp_localize_script( 'ure-js-exp-imp', 'ure_data_exp_imp', 
                array(
                    'export_roles' => __('Export', 'ure'),
                    'import_roles' => __('Import', 'ure'),
                    'import_roles_title' => __('Import Roles', 'ure'),
                    'select_file_with_roles' => __('Select file with roles data', 'ure')
                ));
    }
    // end of add_js()
    ...
    public function export_roles() {        
        ...
        $timestamp = date('_d-m-Y_h_i_s', time());
...
<div id="ure_import_roles_dialog" class="ure-modal-dialog">
  ...
  <div style="padding:10px;">
    <div class="ure-label"><?php echo __('Select file with roles: ', 'ure'); ?></div>
...
?>
			<div style="clear: left; display:block; margin-left:10px; <?php echo $fontColor;?>" id="ure_import_to_all_div">
					...
					<label for="ure_import_to_all" title="<?php echo $hint;?>"><?php _e('Apply to All Sites', 'ure');?></label>
			</div>
<?php
}		
?>
...
<?php        
    ...
    public function import_roles() {        
        ...
        if (empty($upload_dir['error'])) {           
            ...
            $result->message = 'File upload error. '.$upload_dir['error'];
        }
    ...
    protected function validate_role($role_id, $role) {
        ...
        if ($role_id!==$sanitized_role_id) {
            $result->message = __('Import failure: role ID contains invalid characters.', 'ure');            
        } elseif (!is_array($role)) {
            $result->message = __('Import failure: role should have an array structure.', 'ure');            
        } elseif (count($role)!==2) { // only two items 'name' and 'capabilities' should be in role array
            $result->message = __('Import failure: role '.$role_id.' array is not valid (> 2 items).', 'ure');            
        } elseif (!isset($role['name']) || !isset($role['capabilities'])) {   // wrong role array structure
            $result->message = __('Import failure: Wrong role '.$role_id.' array structure: name and capabilities not found.', 'ure');
        } elseif ($role['name']!==($name = sanitize_text_field($role['name']))) {  // wrong characters in the role name
            $result->message = __('Import failure: Wrong characters in the role name - sanitized version:', 'ure').' '.$name;        
        } elseif (!is_array($role['capabilities'])) {
            $result->message = __('Import failure: role capabilities should have an array structure.', 'ure');
        } else {
            $result->success = true;
        }
    ...
    protected function validate_capabilities($role_id, $capabilities) {
        ...
        foreach($capabilities as $key=>$value) {
            ...
            if ($key!==$sanitized_key) {    // illegial character found at the capability identifire
                $result->message = __('Import failure: Wrong characters in the capability ID - sanitized version:', 'ure').' '.$sanitized_key;
                break;
            } elseif (!is_bool($value)) {
                $result->message = __('Import failure: Wrong capability value - only 1 and 0 are allowed.', 'ure');
                break;
            } else {
                $result->success = true;
            }
    ...
    private function validate_roles() {
        ...
        if (!is_array($this->roles) || count($this->roles)==0) {   // not valid roles array
            $result->message = __('Import failure: Roles file is broken possibly.', 'ure');
            return $result;
        }
    ...
    public function import_roles_notification() {
        if ($_REQUEST['action'] == 'roles_import_note') {             
            if ($_REQUEST['result']==1) {
                $this->lib->notification = __('Roles are imported successfully', 'ure');
            } elseif (!empty($_REQUEST['message'])) {
                $this->lib->notification = sanitize_text_field($_REQUEST['message']);
            } else {
                $this->lib->notification = __('Unknown error: Roles import was failed', 'ure');
            }
        }
    }
