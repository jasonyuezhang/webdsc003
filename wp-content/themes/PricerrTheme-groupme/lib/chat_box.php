...
{
    return __("Chat Box", 'PricerrTheme') . " - ";
}
...
?>
    ...
    <div
        class="box-title"><?php echo sprintf(__("Conversation #%s - %s", 'PricerrTheme'), $orderid, PricerrTheme_wrap_the_title($post_a->post_title, $pid)); ?></div>
    ...
    foreach ($r as $row) {
        ...
        if ($row->uid > 0) {
            ...
            {
                $icon = get_bloginfo('template_url') . '/images/handshake.png';
                $message = __('The seller has requested a mutual cancellation for this order.', 'PricerrTheme');
            }

            if ($row->uid == -9) //the buyer requested cancellation
            {
                $icon = get_bloginfo('template_url') . '/images/handshake.png';
                $message = __('The buyer has requested a mutual cancellation for this order.', 'PricerrTheme');
            }
        ...
        <div class="padd10">
            <form method="post" enctype="multipart/form-data">
                <table width="100%">
                    ...
                    <tr>
                        <td valign="top"></td>
                        <td><input class="button btn-medium-large rounded grey" type="submit" name="send-message" value="<?php _e('Send', 'PricerrTheme'); ?>"/></td>
                    </tr>
