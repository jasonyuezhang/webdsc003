...
function PricerrTheme_add_theme_styles()
{
    ...
    wp_enqueue_script('jqueryui_core', 'http://jquery-ui.googlecode.com/svn/tags/latest/ui/jquery.effects.core.js', array('jquery'));
    wp_enqueue_script('jqueryui_slide', 'http://jquery-ui.googlecode.com/svn/tags/latest/ui/jquery.effects.slide.js', array('jquery'));
...
function PricerrTheme_my_jobs_columns($columns) //this function display the columns headings
{
    $columns = array(
        ...
        "title" => __("Group Name", 'PricerrTheme'),
...
function PricerrTheme_set_metaboxes()
{
    ...
    add_meta_box('job_images', 'Group Images', 'PricerrTheme_theme_job_images', 'job', 'advanced', 'high');
    add_meta_box('job_extra', 'Group Additional Services', 'PricerrTheme_theme_job_additional', 'job', 'advanced', 'high');
    add_meta_box('job_dets', 'Group Details', 'PricerrTheme_theme_job_dts', 'job', 'side', 'high');
...
function PricerrTheme_theme_job_additional()
{
    ...
    <table width="100%">
        ...
        for ($k = 1; $k <= $sts; $k++) {
            ...
            <tr>
                <td width="200">
                    <?php _e('For an extra', 'PricerrTheme'); ?><input type="text" size="3" name="extra<?php echo $k; ?>_price" value="<?php echo get_post_meta($pid, 'extra' . $k . '_price', true); ?>"/><?php echo PricerrTheme_get_currency(); ?>&nbsp;&nbsp;<?php _e('I will', 'PricerrTheme'); ?>:
                </td>
                <td>
...
if (!function_exists('PricerrTheme_theme_job_dts')) {
    function PricerrTheme_theme_job_dts()
    {
        ...
        <ul id="post-new4">
            ...
            <li>
                <h2><?php echo __('Membership Price', 'PricerrTheme'); ?>:</h2>
                <p>
                    <?php
                    $PricerrTheme_enable_dropdown_values = get_option('PricerrTheme_enable_dropdown_values');
                    $PricerrTheme_enable_free_input_box = get_option('PricerrTheme_enable_free_input_box');
...
function PricerrTheme_create_post_type()
{
    ...
    register_post_type('job',
        array(
            'labels' => array(
                'name' => __('Groups', 'PricerrTheme'),
                'singular_name' => __('Group', 'PricerrTheme'),
                'add_new' => __('Add New Group', 'PricerrTheme'),
                'new_item' => __('New Group', 'PricerrTheme'),
                'edit_item' => __('Edit Group', 'PricerrTheme'),
                'add_new_item' => __('Add New Group', 'PricerrTheme'),
                'search_items' => __('Search Groups', 'PricerrTheme')),
            'public' => true,
            'menu_position' => 5,
    ...
    register_taxonomy('job_cat', 'job', array('hierarchical' => true, 'label' => __('Group Categories', 'PricerrTheme')));
    register_taxonomy('job_location', 'job', array('hierarchical' => true, 'label' => __('Group Locations', 'PricerrTheme')));
    ...
    <div class="post_thumb">
        ...
        <div class="gridview_title">
            <div class="padd10_m">
                ...
                echo sprintf(__('%s ... <span class="no_bolds">for %s</span>', 'PricerrTheme'), $string_title, $prc);
                echo '</a>';
        ...
        <div class="post" id="post-<?php the_ID(); ?>">
            ...
            <div class="padd10_1">
                <div class="image_holder">
                    <a href="<?php the_permalink(); ?>">
                        <img width="102" height="72" class="<?php echo $img_class; ?>" src="<?php echo PricerrTheme_get_first_post_image(get_the_ID(), 102, 72); ?>"/>
                    </a>
                </div>
                <div class="title_holder">
                    <div class="ttl_holder_down">
                        <h2><?php
                            ...
                            else if ($days_needed == 1) echo '<span class="express_job_spn">' . __('Express Group', 'PricerrTheme') . '</span>';
                            ?>
                            <a class="title_of_job" href="<?php the_permalink() ?>" rel="bookmark"
                    ...
                    <p class="mypostedon">
                        ...
                        echo sprintf(__('<span class="spn_txt_diff">Author</span>: <a href="%s" class="title_of_job2">%s</a>', 'PricerrTheme'), PricerrTheme_get_user_profile_link($usr->user_login), $usr->user_login);
                        echo '</span>';
                        //echo '<span class="inline">';
                ...
                <div class="order_now_new_btn">
                    <a href="<?php echo get_permalink($pid); ?>" class="button btn-medium-large rounded grey"><?php
                        echo sprintf(__('Group Me', 'PricerrTheme')); ?></a>
                </div>
            </div>
        </div>
...
if (!function_exists('PricerrTheme_get_categories_click')) {
    function PricerrTheme_get_categories_click($taxonomy, $selected = "", $include_empty_option = "", $classes = "", $attributes = "")
    {
        $args = "orderby=name&order=ASC&hide_empty=0&parent=0";
        $terms = get_terms($taxonomy, $args);

        $ret = '<select name="' . $taxonomy . '_cat" class="' . $classes . '" id="' . $classes . '" ' . $attributes . '>';
        if (!empty($include_empty_option)) $ret .= "<option value=''>" . $include_empty_option . "</option>";
...
if (!function_exists('PricerrTheme_get_subcategories')) {
    function PricerrTheme_get_subcategories($cat_id, $selected = "", $include_empty_option = "")
    {
        ...
        $ret = '<div class="post-new-group"><div class="col-sm-3"><div class="post-new-label"><label>'
            . __('Subcategory', 'PricerrTheme') . '</label></div></div><div class="col-sm-9"><div>';
...
function PricerrTheme_framework_init_widgets()
{
    ...
    register_sidebar(array(
        'name' => __('PricerrTheme - Group Single Sidebar', 'PricerrTheme'),
        'id' => 'job-widget-area',
            ...
            <ul class="xoxo">
                <li class="widget-container widget_text" id="my-account-menu">
                    <ul id="my-account-admin-menu">
                        <li>
                            <a href="<?php echo get_permalink($account_id); ?>" <?php if ($account_id == $my_pid) echo "class='active_link'"; ?>><?php _e("My Groups", 'PricerrTheme'); ?></a>
                        </li>
                        <li>
                            <a href="<?php echo PricerrTheme_post_new_link(); ?>" class="post-new-btn <?php if ($post_new_id == $my_pid) echo "active_link"; ?>"><?php _e("Post New Group", 'PricerrTheme'); ?></a>
                        </li>
                        <li>
                            <a href="<?php echo get_permalink($shopping_id); ?>" <?php if ($shopping_id == $my_pid) echo "class='active_link'"; ?>><?php _e("Joined Groups", 'PricerrTheme'); ?></a>
                        </li>
                        <li>
                            <a href="<?php echo get_permalink($sales_id); ?>" <?php if ($sales_id == $my_pid) echo "class='active_link'"; ?>><?php _e("Manage Groups", 'PricerrTheme'); ?></a>
                        </li>
                        <li>
    ...
    <?php endif; ?>
        ...
        <div class="padd10_only">
            ...
            <div class="title_holder3"><h2><a href="<?php
                ...
                bloginfo('siteurl'); ?>/?jb_action=delete_job&jobid=<?php the_ID(); ?>" class="del_job"><?php _e('Delete Group', 'PricerrTheme'); ?></a><a href="<?php
                bloginfo('siteurl'); ?>/?jb_action=edit_job&jobid=<?php the_ID(); ?>" class="edit_job"><?php _e('Edit Group', 'PricerrTheme'); ?></a><?php
                if ($active == "1"): ?><a href="<?php bloginfo('siteurl'); ?>/?jb_action=deactivate_job&jobid=<?php the_ID(); ?>" class="deactivate_job"><?php
                    _e('Deactivate Group', 'PricerrTheme'); ?></a><?php else: ?><a href="<?php
                bloginfo('siteurl'); ?>/?jb_action=activate_job&jobid=<?php the_ID(); ?>" class="deactivate_job"><?php _e('Activate Group', 'PricerrTheme'); ?></a><?php
                endif;
                global $post;
                if ($featured == "0" or ($featured == "1" and $post->post_status != "publish")):
                    ...
                    ?><a href="<?php echo $rdrlnk; ?>" class="feature_job"><?php _e('Feature Group', 'PricerrTheme'); ?></a><?php
                endif;
                ...
                if ($post->post_status == "draft" && $featured == "1" && $paid == "0")
                    echo '<div class="not-published-yet">' . sprintf(__('Your group is not published yet. Please <b class="pay-feature"><a href="%s">pay the featured fee</a></b> to publish.', 'PricerrTheme'), $rdrlnk) . '</div>';
                ?></div></div></div><?php
    }
}
/*************************************************************
    ...
    function PricerrTheme_small_post()
    {
        ...
        <div class="post" id="post-<?php the_ID(); ?>">
            <div class="image_holder2">
                ...
                <a href="<?php the_permalink(); ?>">
                    <img width="62" height="50" class="image_class"
    ...
    function PricerrTheme_show_bought($row)
    {
        ...
        <div class="image_holder3">
            <a href="<?php echo get_permalink($pid); ?>">
                <img width="65" height="50" class="<?php echo $img_class; ?>" src="<?php echo PricerrTheme_get_first_post_image($pid, 65, 50); ?>"/>
            </a>
        </div>
        ...
        <div class="sold_on"><?php
            ...
            &diams; <?php echo sprintf(__("Expected Delivery: %s", "PricerrTheme"), $expected); ?>
        ...
        <?php if ($completed == 0 && $done_seller == 1 && $closed != 1): ?>
            ...
            echo $row->id; ?>" class="show_status"><?php
                _e("Mark Completed", "PricerrTheme"); ?></a>
            ...
            if ($row->request_cancellation_from_seller == 0 and $row->request_cancellation_from_buyer == 0):
                ...
                if ($row->accept_cancellation_request != -1):
                    ...
                    if ($row->request_cancellation_from_seller == 1):
                        ...
                        printf(__('The seller of this job has requested a mutual cancellation for this job on %s. <br/>Seller\'s Message: <span class="en_em">%s</span> <br/><br/> <b><a href="%s">Answer this request</a></b>.', 'PricerrTheme'), $dt, $ss, $lnk7);
                        ?><br/><br/><?php endif;
                    ...
                    if ($row->request_cancellation_from_buyer == 1):
                        ...
                        printf(__('You have requested a mutual cancellation on this job on %s. Waiting the seller\' reply.', 'PricerrTheme'), $dt);
                        echo '<br/><br/>';
                    endif;
                endif;
            endif;
        ...
        echo $post->post_author; ?>" class="show_status"><?php _e("Contact Seller", "PricerrTheme");

            ?></a><a href="<?php bloginfo('siteurl'); ?>/?jb_action=chat_box&oid=<?php
        echo $row->id; ?>" class="show_buyer_notes" rel="<?php
        echo $id; ?>"><?php _e("Conversation Page", "PricerrTheme"); ?></a><br/><br/><?php
...
if (!function_exists('PricerrTheme_show_sale')) {
    function PricerrTheme_show_sale($row)
    {
        ...
        <div class="image_holder3">
            <a href="<?php the_permalink(); ?>">
                <img width="65" height="50" class="<?php echo $img_class; ?>" src="<?php
        ...
        <div class="title_holder3">
            <h2><?php
                ...
                if ($days_needed == 1) echo '<span class="express_job_spn">' . __('Express Group', 'PricerrTheme') . '</span>';
                if ($instant == 1) echo '<span class="instant_job_spn">' . __('Instant Delivery', 'PricerrTheme') . '</span>';
                ...
                ?></a></h2><div class="sold_on"><?php echo sprintf(__("Sold on: %s", "PricerrTheme"), $sold);
            ...
            if ($delivered == 0 && $closed != 1):
                ...
                echo $row->id; ?>"><?php _e("Cancel Order", "PricerrTheme"); ?></a><?php endif;
            elseif ($completed != 1 && $closed != 1):

                ?><span style="font-size:10px;"><em> <?php _e('Waiting for the buyer to confirm.', 'PricerrTheme'); ?></em></span><?php
            ...
            $instruction_box = get_post_meta($pid, 'instruction_box', true);
            if (!empty($instruction_box)) {
                echo sprintf(__('<br><br>Instructions for the buyer: %s', 'PricerrTheme'), $instruction_box);
            ...
            ?></div></div><div class="close_order_div" id="cancel_order_div_id_<?php echo $row->id; ?>"><div class="padd10">

                <!-- <div class="box-title-special"><?php _e('Request Cancellation', 'PricerrTheme'); ?></div>
                ...
                <div class="box-title-special"><?php _e('Force Cancellation', 'PricerrTheme'); ?></div>
                <form method="post" action="<?php bloginfo('siteurl'); ?>/?jb_action=cancel_job_request">
                    <input type="hidden" value="<?php echo $row->id; ?>" name="orderid"/>
                    <table>
                        <tr>
...
function PricerrTheme_register_new_user_sitemile($user_login, $user_password, $user_reppassword, $user_email)
{
    ...
    // TODO: modify register conformation function and email, maybe remove password from the email
...
function PricerrTheme_get_post_take_space($pid)
{
    return get_post_meta($pid, 'space_take', true);
}

function PricerrTheme_get_post_location($pid)
{
    return get_post_meta($pid, 'job_location', true);
}
...
function PricerrTheme_get_post_space_take($pid)
{
    $sp = get_post_meta($pid, 'space_take', true);
...
if (!function_exists('PricerrTheme_is_login_page')) {
    function PricerrTheme_is_login_page() {
        return (PricerrTheme_get_current_url() == esc_url(wp_login_url()));
    }
}
            ...
            jQuery('.like_this_job').click(function () {
                ...
                jQuery.ajax({
                    ...
                    success: function (msg) {
                        ...
                        //alert("asd");
                        jQuery("#lk-stuff" + pid).html('<a href="#" class="unlike_this_job" rel="' + pid + '"><?php echo addslashes(__("Unlike this Group","PricerrTheme")); ?></a>');
            ...
            jQuery('.unlike_this_job').click(function () {
                ...
                jQuery.ajax({
                    ...
                    success: function (msg) {
                        ...
                        jQuery("#lk-stuff" + pid).html('<a href="#" class="like_this_job" rel="' + pid + '"><?php echo addslashes(__("Like this Group","PricerrTheme")); ?></a>');
