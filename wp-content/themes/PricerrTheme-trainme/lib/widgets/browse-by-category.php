...
add_action('widgets_init', 'register_browse_by_category_widget_job');
function register_browse_by_category_widget_job()
{
    register_widget('PricerrTheme_browse_by_category');
}

class PricerrTheme_browse_by_category extends WP_Widget
{

    function PricerrTheme_browse_by_category()
    {
        ...
        $this->WP_Widget('browse-by-category', 'PricerrTheme - Browse by Category', $widget_ops, $control_ops);
    }
    ...
    function widget($args, $instance)
    {
        ...
        $terms = get_terms("job_cat", "parent=0&hide_empty=0" . $jk);
        ...
        if ($count > 0) {
            ...
            if ($count > 0) {
                ...
                foreach ($terms as $term) {
                    ...
                    $total_ads = PricerrTheme_get_custom_taxonomy_count2('job', $term->slug, 'job_cat'); //PricerrTheme_get_custom_taxonomy_count('job',$term->slug);
					...
					';
                        foreach ($terms2 as $term2) {
                            ++$cnt;
                            $tt = PricerrTheme_get_custom_taxonomy_count2('job', $term2->slug, 'job_cat'); //PricerrTheme_get_custom_taxonomy_count('job',$term2->slug);
						...
						";
                            ...
                            if ($terms3) {
                                ...
                                foreach ($terms3 as $term3) {
                                    ++$cnt;
                                    $tt = PricerrTheme_get_custom_taxonomy_count2('job', $term3->slug, 'job_cat'); //PricerrTheme_get_custom_taxonomy_count('job',$term3->slug);
								...
								";
                                    ...
                                    if ($terms4) {
                                        ...
                                        foreach ($terms4 as $term4) {
                                            ++$cnt;
                                            $tt = PricerrTheme_get_custom_taxonomy_count2('job', $term4->slug, 'job_cat');
                                            $total_ads += $tt;
