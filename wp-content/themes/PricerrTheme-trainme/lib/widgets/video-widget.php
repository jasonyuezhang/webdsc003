<?php
add_action('widgets_init','fitness_video_load_widgets');


function fitness_video_load_widgets(){
		register_widget("Fitness_Video_Widget");
}

class Fitness_Video_Widget extends WP_widget{
	
	function Fitness_Video_Widget(){
		$widget_ops = array('classname' => 'fitness_video_widget', 'description' => 'Video Widget');

		$control_ops = array('id_base' => 'fitness_video_widget');

		$this->WP_Widget('fitness_video_widget', 'Fitness : Video Widget', $widget_ops, $control_ops);
	...
	function widget($args,$instance){
		...
		?>
		<div class="preview-frame embed-container">
		<?php if($type == 'Youtube') { ?>
			<iframe src="http://www.youtube.com/embed/<?php echo $id; ?>?rel=0" frameborder="0" allowfullscreen></iframe>
		<?php } elseif($type == 'Vimeo') { ?>
		<iframe src="http://player.vimeo.com/video/<?php echo $id; ?>?title=0&amp;byline=0&amp;portrait=0&amp;color=D54E21" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe>
