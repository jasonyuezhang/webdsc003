...
if (!function_exists('PricerrTheme_my_account_shopping_area_function')) {
            ...
            <div class="my_box3">
                <div class="padd10">
                    <div class="shopping_menu_dv">
                        <ul id="shopping_menu">
                            <li><a <?php
                                echo($pg == "home" ? 'class="actiove"' : ""); ?> href="<?php echo $shp_pg_lnk; ?>"><?php
                                    echo sprintf(__("Active Lessons (%s)", "PricerrTheme"), $act_nr); ?></a>
                            </li>
                            <li><a <?php
                                echo($pg == "pending" ? 'class="actiove"' : ""); ?> href="<?php echo $shp_pg_lnk; ?>pg=pending"><?php
                                    echo sprintf(__("Pending my Review (%s)", "PricerrTheme"), $rev_nr); ?></a>
                            </li>
                            <li><a <?php
                                echo($pg == "cancelled" ? 'class="actiove"' : ""); ?> href="<?php echo $shp_pg_lnk; ?>pg=cancelled"><?php
                                    echo sprintf(__("Cancelled (%s)", "PricerrTheme"), $can_nr); ?></a>
                            </li>
                            <li><a <?php
                                echo($pg == "completed" ? 'class="actiove"' : ""); ?> href="<?php echo $shp_pg_lnk; ?>pg=completed"><?php
                                    echo sprintf(__("Completed (%s)", "PricerrTheme"), $com_nr); ?></a>
                            </li>
                        </ul>
                    </div>
			 ...
			 order by id desc";
                            ...
                            if (count($r) == 0) echo __('No active lesson', 'PricerrTheme');
                            else {
                                foreach ($r as $row) {
                            ...
                            global $wpdb;
                            $prefix = $wpdb->prefix;
                            $s = "select * from " . $prefix . "job_orders where uid='$uid' AND done_seller='1' AND done_buyer='0' AND closed='0' order by id desc";
                            $r = $wpdb->get_results($s);
