...
if (isset($_GET['confirm'])):
    ...
    if (count($r1) == 0) {
        ...
        if ($ttotal > $crds) {
            echo 'post meta: '.get_post_meta($pid, 'job_cost', true);
            echo '<br>PID: '.$pid;
            echo '<br>Shipping: '.$shipping.' Extra: '.$extr_ttl.' Price'.$price;
            echo '<br>Total: '.$ttotal.' Cards: '.$crds. 'NO_CREDITS_LEFT';
            exit;
        }
        ...
        PricerrTheme_send_email($admin_email, sprintf(__('New Lesson Purchased on your site - %s', 'PricerrTheme'), $post->post_title), $message);
        ...
        <div class="my_box3">
            <div class="padd10">
                ...
                <div class="box-content clear">
                    ...
                    <br/><br/>
                    <?php echo '<a class="show_status" href="' . get_bloginfo('siteurl') . '/?pay_for_item=credits&jobid=' . $pid . '&confirm=1&extras=' . $_GET['extras'] . '">' . __('Confirm Purchase', 'PricerrTheme') . '</a>'; ?>
                </div>
            </div>
        </div>
    </div>
