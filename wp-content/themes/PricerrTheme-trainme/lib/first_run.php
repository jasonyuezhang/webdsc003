...
if (is_admin() && 'themes.php' == $pagenow && isset($_GET['activated'])) {
    ...
    PricerrTheme_insert_pages('PricerrTheme_post_new_page_id', 'Post New Lesson', '[pricerr_theme_post_new]');
    ...
    update_option('PricerrTheme_right_side_footer', '<a title="Jason Zhang" href="http://yuezhang.org">Micro Job Pricerr Theme</a>');
...
if (empty($opt3_a)) {
    ...
    PricerrTheme_insert_pages('PricerrTheme_all_categories_page_id', 'All Categories', '[pricerr_theme_all_categories]');
    PricerrTheme_insert_pages('PricerrTheme_all_locations_page_id', 'All Locations', '[pricerr_theme_all_locations]');
    PricerrTheme_insert_pages('PricerrTheme_advanced_search_id', 'Search Lessons', '[pricerr_theme_search_jobs]');
...
if (empty($opt2)) {
    ...
    if (have_posts()) :
        while (have_posts()) : the_post();
            ...
            $ttl = get_the_title();
            $newttl = get_post_meta(get_the_ID(), 'title_variable', true); //title_variable PricerrTheme_reomve_i_will($ttl, get_post_meta(get_the_ID(),'job_cost', true));
