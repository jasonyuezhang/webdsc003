...
function PricerrTheme_new_user_notification($user_id, $plaintext_pass = '')
{
    $user = new WP_User($user_id);

    $subject = get_option('PricerrTheme_new_user_email_subject');
    $message = get_option('PricerrTheme_new_user_email_message');
    ...
    PricerrTheme_send_email($user_email, $subject, $message);
...
if (!function_exists('my_retrieve_password')) :
  function my_retrieve_password()
  {
    ...
    $message = __('Someone requested that the password be reset for the following account:', $current_theme_locale_name) . "<br>" . "\r\n\r\n";
    // $message .= get_site_url() . "<br>" . "\r\n\r\n";
    ...
    //$message .= "key : " . $key . " user_login : " . rawurlencode($user_login) . " blog ID : " . get_current_blog_id() . "<br>" ;
    $message .= get_site_url(get_current_blog_id(), "wp-login.php?action=rp&key=" . $key . "&login=" . rawurlencode($user_login)) . "<br>" . "\r\n";
    ...
    if ($message && !wp_mail($user_email, $title, $message))
        wp_die(__('The e-mail could not be sent.', $current_theme_locale_name) . "<br />\n"
          . __('Possible reason: your host may have disabled the mail() function...', $current_theme_locale_name));
