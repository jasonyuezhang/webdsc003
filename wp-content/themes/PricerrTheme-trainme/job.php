...
<?php if (have_posts()) while (have_posts()) :
        ...
        if ($active == 0) {
            ...
            <div class="ad_page_title_purchase">
                <div class="ad_page_title">
                    <div class="image_and_user">
                        ...
                        ?>"/>
                        <a href="<?php echo PricerrTheme_get_user_profile_link($user->user_login); ?>"><?php echo $user->user_login; ?></a>:
                    </div>
                    <h1><?php the_title(); ?></h1>
                </div>
            </div>
            <div class="job_info_stuff">
                <div class="padd5">
                    <!--<div class="job_rating"><?php _e("GROUP RATING", "PricerrTheme"); ?>:</div>-->
                    <!--<div class="rating_100"><?php echo PricerrTheme_get_job_rating($pid); ?>%</div>-->
                    ...
                    <div class="rating_100"><?php echo PricerrTheme_get_avg_lesson_rating($post->post_author); ?>%</div>
            ...
            <div class="clear10">
                <div class="ad_page_content_stuff">
                    <div class="jb-page-image-holder">
                        ...
                        <?php endif; ?>
                        <img class="img_class" src="<?php echo PricerrTheme_get_first_post_image($pid, 315, 190); ?>" alt="<?php the_title(); ?>"/>
                        <?php
                        ...
                        if ($arr) {
                            ...
                            foreach ($arr as $image) {
                                echo '<li><a href="' . PricerrTheme_generate_thumb($image, 800, 600) . '" rel="image_gal1">'
                                  . '<img src="' . PricerrTheme_generate_thumb($image, 76, 55) . '" class="img_class" /></a></li>';
                            }
                            echo '</ul>';
                        }
                    ...
                    <div class="job-page-details-holder">
                        <div class="job-page-date-holder">
                            <span><?php echo PricerrTheme_get_post_start_datetime($pid) . " - " . PricerrTheme_get_post_end_datetime($pid); ?></span>
                        </div>
                        <?php the_content(); ?>
            ...
            <div class="job_info_stuff2">
                <div class="padd5">
                    <div class="job_rating4"><?php
                        ...
                        $likes = PricerrTheme_get_likes_nr($pid);
                        $sc = $tm - $tm2;
                        echo sprintf(__("created %s ago", "PricerrTheme"), PricerrTheme_prepare_seconds_to_words($sc));
                    ?></div>
                    <div class="separator_job_rating2"></div>
                    <div class="job_rating4"><?php echo sprintf(__("%s ratings for lesson", "PricerrTheme"), $total_ratings_pid); ?></div>
                    <!--<div class="separator_job_rating2"></div>-->
                    <!--<div class="job_rating4"><?php printf(__("%s likes for this lesson", "PricerrTheme"), $likes); ?></div>-->
                    <!--<div class="separator_job_rating2"></div>-->
                    <!--<div class="job_rating4 like-stuff" id="lk-stuff<?php the_ID(); ?>"><?php
                    /*
                        global $wpdb;
                        global $current_user;
                        get_currentuserinfo();
                        $uid = $current_user->ID;
                        $s = "select * from " . $wpdb->prefix . "job_likes where pid='" . $pid . "' AND uid='$uid'";
                        $r = $wpdb->get_results($s);

                        if (!is_user_logged_in()) { ?>
                            <a href="<?php PricerrTheme_login_url_with_redirect() ?>" class="like_this_job_no_logged" rel="<?php the_ID(); ?>">
                            <?php _e("Like this Lesson", "PricerrTheme"); ?>
                            </a>
            ...
            $vid = 0;
            for ($i = 1; $i <= 3; $i++) {
                $video = get_post_meta($pid, 'youtube_link' . $i, true);
                ...
                if (!empty($video) && $done == 1) {
                    ...
                    echo '<iframe style="margin-right:10px;margin-bottom:10px" width="225" height="180" src="http://www.youtube.com/embed/' .
                        $code_here . '" frameborder="0" allowfullscreen></iframe> ';
                    $vid = 1;
                }
            }

            if ($vid == 0) {
                _e('No videos attached with this lesson.', 'PricerrTheme');
            }?>
            </div>
            <div class="clear10"></div>
                        ...
                        <div class="padd10_only">
                            ...
                            <div class="title_holder4">
                                ...
                                <div class="c111"><p><?php
                                    ...
                                    echo stripslashes($row->reason); ?></p>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        if ($post->post_author != $current_user->ID):
            ...
            <div class="ad_page_purchase">
                ...
                <form method="get" name="myFormPurchase" class="fltRight" action="<?php echo $act; ?>">
                            ...
                            <div class="padd10">
                                <ul class="purchase_extra_ul">
                                    ...
                                    foreach ($extra_job_add as $extra_job_add_item):
                                        echo '<li><input type="checkbox" name="extra' . $extra_job_add_item['extra_nr'] . '" value="1" />' . __('I will provide', 'PricerrTheme') . ' ' . $extra_job_add_item['content'] . ' ' . __('for', 'PricerrTheme') . '
                    ...
                    ?>
                    <p class="contact-seller-wrap"><a href="<?php echo $privurl_m; ?>priv_act=send&pid=<?php the_ID(); ?>&uid=<?php echo $author->ID; ?>" class="contact-seller-thing"><?php _e("Contact Seller", "PricerrTheme"); ?></a></p>
                </form>
            </div>
        <?php endif;
