...
if ($job_category != "all" and $job_tax == "category") {
    ...
    $posted_jobs = sprintf(__('All %s Lessons', 'PricerrTheme'), $term_ct->name);
} elseif ($job_category != "all" and $job_tax == "location") {
    $term_ct = get_term_by('slug', $job_category, 'job_location');
    $posted_jobs = sprintf(__('All Lessons in %s', 'PricerrTheme'), $term_ct->name);
} else {
    $posted_jobs = __('New Lessons', 'PricerrTheme');
}
...
$args = array('posts_per_page' => $nrpostsPage, 'paged' => $pj, 'post_type' => 'job', 'order' => "DESC", 'meta_query' => $meta_querya, 'meta_key' => 'start_date', 'orderby' => 'date');
$the_query = new WP_Query($args);

//echo "hahaha".$wp_query;
echo "<div class='latest-posted-jobs-list'>";
...
<?php endwhile;
    ...
    if ($PricerrTheme_show_pagination_homepage != "yes"):
        ...
        <div class="show-all-jobs white-wrapper">
            <a href="<?php echo get_post_type_archive_link('job') ?>"><?php _e('Show All Lessons', 'PricerrTheme'); ?></a>
        </div>
...
<?php else : ?> <?php $no_p = 1; ?>
    <div class="padd100"><p class="center"><?php _e("Sorry, there are no posted lessons yet", 'PricerrTheme'); ?>.</p>
    </div>
