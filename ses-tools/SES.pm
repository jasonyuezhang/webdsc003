...
sub read_credentials {
    ...
    die "Cannot find AWS_ACCESS_KEY_ID environment variable." unless defined($aws_access_key_id);
 
    $aws_secret_access_key = $ENV{'AWS_SECRET_ACCESS_KEY'};             
    die "Cannot find AWS_SECRET_ACCESS_KEY environment variable." unless defined($aws_secret_access_key);
...
# Turn a string containing data separated by a delimiter into a list
...
sub get_response_flag {
    my $content = shift;
    
    if ($content =~ /<Message>(.*?)<\/Message>/s) {
	switch ($1) {
	    case /Maximum sending rate exceeded/                       { return 'THROTTLING_MAX_RATE';    }
	    case /Daily message quota exceeded/                        { return 'THROTTLING_DAILY_QUOTA'; }
	    case /Maximum number of verified email addresses exceeded/ { return 'THROTTLING_MAX_VERIFY';  }
	    case /Rate exceeded/                                       { return 'THROTTLING_GLOBAL_RATE'; }
	}
    }
...
sub call_ses {
    ...
    my $response_flag = get_response_flag($response->content);
    my $next_token;
    if ($response->content =~ /<NextToken>(.*?)<\/NextToken>/s) {
        $next_token = $1;
    }
    return ($response->code, $response->content, $response_flag, $next_token);
}
