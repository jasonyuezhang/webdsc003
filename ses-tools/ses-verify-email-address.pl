...
# and send the email.
print "Warning: ses-verify-email-address.pl is deprecated. Please use ses-verify-identity.pl instead.\n";
parse_args;
validate_opts;
prepare_params;
...
B<ses-verify-email-address.pl> [B<--help>] [B<-e> URL] [B<--verbose>] B<-v> EMAIL | B<-l> | B<-d> EMAIL
